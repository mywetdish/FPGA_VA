...
#include "../inc/va_lib.h"

uint16_t device_num;
device_info devices[DEVICES_MAX];

qdma_dev_ptr rte_dev = va_init(TEST_DEV_ID,TEST_RING_DEPTH,TEST_BUFF_SIZE);

void va_deinit(struct rte_device *rte_dev) {
    int ret = 0;
    uint16_t dev_id = TEST_DEV_ID;
...
qdma_dev_ptr va_init(uint16_t dev_id, uint32_t ring_depth, uint32_t buff_size) {
    ...
    */
    ...
    rte_dev = rte_pmd_qdma_get_device(dev_id);
    ...
    return qdma_dev_ptr(rte_dev, &va_deinit);
}
